...
https://www.youtube.com/watch?v=O1jpck31Ask
...
def arp_scan(ip_range):
    """
    We use the arping method in scapy. It is a better implementation than writing your own arp scan.
    You'll often see that your own arp scan doesn't pick up
    mobile devices. You can see the way scapy implemented the function here:
...
def is_gateway(gateway_ip):
    ...
    for row in result:
        # We look to see if the gateway_ip is in the row, if it is we return True.
        # If False program continues flow and returns False.
...
def get_interface_names():
    ...
    # We use the listdir() function from the os module. Since we know there won't
    # be files and only directories with the interface names we can save the output as the interface names.
...
def gateway_info(network_info):
    """
    We can see the gateway by running the route -n command. This get us the gateway information.
    We also need the name of the interface for the sniffer function.
    ...
    for iface in network_info:
        for row in result:
            # We want the gateway information to be saved to list called gateways.
            # We know the ip of the gateway so we can compare and see in which row it appears.
            if iface["ip"] in row:
...
def clients(arp_res, gateway_res):
    ...
    Generally you did get the ARP response from the gateway at the 0 index
    but I did find that sometimes this may not be the case.
    Arguments: arp_res (The response from the ARP scan), gateway_res (The response from the gateway_info function.)
    """
    # In the menu we only want to give you access to the clients whose arp tables you want to poison.
    # The gateway needs to be removed.
    client_list = []
...
def allow_ip_forwarding():
    """
    Run this function to allow ip forwarding. The packets will flow through your machine,
    and you'll be able to capture them. Otherwise user will lose connection.
    """
    # You would normally run the command sysctl -w net.ipv4.ip_forward=1 to enable ip forwarding.
    # We run this with subprocess.run()
...
def arp_spoofer(target_ip, target_mac, spoof_ip):
    """
    To update the ARP tables this function needs to be ran twice.
    Once with the gateway ip and mac, and then with the ip and mac of the target.
    Arguments: target ip address, target mac, and the spoof ip address.
    """
    # We want to create an ARP response, by default op=1 which is "who-has" request,
    # to op=2 which is a "is-at" response packet.
    # We can fool the ARP cache by sending a fake packet saying that we're at the router's ip to the target machine,
    # and sending a packet to the router that we are at the target machine's ip.
...
def packet_sniffer(interface):
    """
    This function will be a packet sniffer to capture all the packets sent
    to the computer whilst this computer is the MITM."""
    # We use the sniff function to sniff the packets going through the gateway interface.
    # We don't store them as it takes a lot of resources. The process_sniffed_pkt is a callback function
    # that will run on each packet.
...
allow_ip_forwarding()  # TODO: Get working on windows (as well as linux? code based on os???)
...
# Change the directory again to the directory which contains the script,
# so it is a place where you have write privileges,
os.chdir(cwd)

# Run the packet sniffer on the interface. So we can capture all the
# packets and save it to a pcap file that can be opened in Wireshark.
